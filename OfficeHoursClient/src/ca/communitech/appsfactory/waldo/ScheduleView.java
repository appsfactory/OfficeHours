...
public class ScheduleView extends Activity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        new Thread(new Runnable() {
            public void run() {
                HttpResponse responseinfo = getDate();
				try {
					BufferedReader reader = new BufferedReader(new InputStreamReader(responseinfo.getEntity().getContent(), "UTF-8"));
					String json = reader.readLine();
					JSONTokener tokener = new JSONTokener(json);
					JSONObject jsonobject = new JSONObject(tokener);
					Log.i("LOGGED: ", jsonobject.get("date").toString());
